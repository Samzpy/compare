...
class database():
    ...
    def found(self):
        ...
        with self.__connection.cursor() as cursor:
            ...
            for i in result:
                ...
                queryStringList.append({'p':keyword})
        ...
        with self.__connection.cursor() as cursor:
            ...
            if result:
                ...
                if new_price<db_price:
                    ...
                    sql='insert into mall(com_name,com_price,mall_name,com_web_url,mall_detail_id) values ("%s",%s,"%s","%s",%s)' %(item['name'],item['price'],'yahoo購物中心',item['com_url'],item['com_id'])
                    try:
                        cursor.execute(sql)
                        self.__connection.commit()
                    except Exception as e:
    ...
    def deduplication(self,com_id):
        self.__connection.ping()
        with self.__connection.cursor() as cursor:
            sql='select * from mall where mall_detail_id= %s and mall_name="yahoo購物中心"' %com_id
            cursor.execute(sql)
            result=cursor.fetchall()
            if result:
                return result[0]
