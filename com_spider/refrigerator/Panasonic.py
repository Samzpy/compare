...
class PHONE_Spider:
    def __init__(self):
        self.url='https://www.panasonic.com/tw/consumer/refrigerator.html?page={}'
    ...
    def second_parse(self,html):
        ...
        for i in r_list:
            ...
            ml=i.xpath('string(.//span[@class="copy"])')
            data_pn=pn.index("NR")
            if pn[data_pn-1] != " ":
                a=list(pn)
                a.insert(data_pn,' ')
                pn=''.join(a)
            re=pn.split()
            name="PANASONIC冰箱 "+re[1] +' ' +re[0] +' ' +ml
            number=re[1]
            keyword='PANASONIC '+number
            name_list.append(['panasonic','refrigerator',name,number,keyword,img])
        else:
            return name_list
