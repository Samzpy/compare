...
class PHONE_Spider:
    def __init__(self):
        self.url='https://www.panasonic.com/tw/consumer/digital-av/tv.html?browsing=params&sort=Featured&page={}'
    ...
    def second_parse(self,html):
        ...
        for i in r_list:
            url_obj=i.xpath('.//div[@class="common-productbox-product"]/a/@href')[0]
            url='https://www.panasonic.com'+url_obj
            name=self.parse_third(url)
            ...
            name_list.append(['panasonic','tv',name,pn,keyword,img])
        else:
            return name_list
    ...
    def parse_third(self,url):
        html=self.get_html(url)
        if html:
            parse_obj_third=etree.HTML(html)
            name=parse_obj_third.xpath('//*[@id="browsebar-name"]/h1/text()')[0]
            return 'Panasonic '+ name
